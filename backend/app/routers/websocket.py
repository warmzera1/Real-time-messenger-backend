...
async def websocket_endpoint(
    ...
    Подключение: wss://example.com/ws?token=<JWT_TOKEN>
    """
    ...
    try:
        ...
        if not user:
            await websocket.close(code=1008)      # Политика нарушена
            return 
        ...
        logger.info(f"WebSocket отключено нормально")
    except Exception as e:
        logger.error(f"WebSocket ошибка: {e}")
    finally:
        # Гарантированое отключение
